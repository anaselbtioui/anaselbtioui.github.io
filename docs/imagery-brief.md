...
Placeholder plates in [`media/work/`](../media/work/), generated by [`scripts/make-archive-thumbs.py`](../scripts/make-archive-thumbs.py) (PIL). Swap for authored captures once real cases land.

| File | Variant | Role |
|------|---------|------|
| `work-01.gif` + `-poster.webp` | `fig-open` — cut fig, crimson flesh, seed speckle | Slot 01 |
| `work-02.gif` + `-poster.webp` | `iris-bloom` — blue iris, ochre throat | Slot 02 |
| `work-03.gif` + `-poster.webp` | `strata-autumn` — late-autumn ridges, low ochre sun | Slot 03 |
| — | `fig-whole` — unopened fig, nothing revealed | Spare |

Plates sit in a full-bleed horizontal strip (`.work-strip`), arched portrait crop (4:5, domed top, hard bottom edge), aligned on one baseline. GIF plays on hover, on focus, on touch, and on the plate nearest the strip centre; poster is the idle and reduced-motion state. Behaviour lives in [`work-strip.js`](../work-strip.js): mouse drag to pan, arrow / Home / End keys.

Source plates are authored portrait at `W, H = 400, 500` so nothing crops in the arch.

**Rules the variants hold:** organic outlines (no geometric icons), teal field with air around the subject, **one** accent per plate (fig crimson · iris blue-violet · autumn ochre), diagonal light + cast shadow for depth, static grain, under 400KB per GIF at 8 frames ≈ 2.7s.

Preview a single variant: `python3 scripts/make-archive-thumbs.py <slug> <variant>`. Slot assignment lives in `ASSIGNMENT` in that script.
